<?php if ( isset( $query_result ) && intval( $query_result->max_num_pages ) > 1 ) {
	...
	$dots       = '<span class="qodef-m-pagination-item qodef--dots">' . esc_html__( '&hellip;', 'askka' ) . '</span>';
	$set_dots   = false;
	$dots_added = false;
	...
	?>
	<div class="qodef-m-pagination qodef--standard" <?php askka_is_installed( 'framework' ) ? qode_framework_inline_style( $pagination_type_standard_top_margin ) : ''; ?>>
		<div class="qodef-m-pagination-inner qodef-content-grid">
			<nav class="qodef-m-pagination-items">
				<a class="qodef-m-pagination-item qodef--prev <?php echo esc_attr( implode( ' ', $prev_classes ) ); ?>" href="#" data-paged="1">
					<?php askka_render_svg_icon( 'pagination-arrow-left' ); ?>
				</a>
				<?php
				for ( $i = 1; $i <= $total; $i ++ ) {
					$classes     = array();
				...
				<a class="qodef-m-pagination-item qodef--next <?php echo esc_attr( implode( ' ', $next_classes ) ); ?>" href="#" data-paged="2">
					<?php askka_render_svg_icon( 'pagination-arrow-right' ); ?>
				</a>
			</nav>
		</div>
	</div>
